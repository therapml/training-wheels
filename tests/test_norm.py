...
class TestLayerNorm:
    ...
    def test_layernorm_output(self, layernorm_data):
        ...
        np.testing.assert_allclose(
            result_np, expected, rtol=1e-4, atol=1e-6, err_msg="LayerNorm output doesn't match expected computation"
        )
...
class TestRMSNorm:
    ...
    def test_rmsnorm_output(self, rmsnorm_data):
        ...
        np.testing.assert_allclose(
            result_np, expected, rtol=1e-4, atol=1e-6, err_msg="RMSNorm output doesn't match expected computation"
        )
